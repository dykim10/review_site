...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tag-wrap.err { border-color: rgba(248,113,113,0.4) !important; }
        ...
        .pi-img { width: 100%; aspect-ratio: 1; object-fit: cover; border-radius: 6px; border: 1px solid var(--border); display: block; }
        .pi-rm { position: absolute; top: 3px; right: 3px; width: 19px; height: 19px; background: rgba(0,0,0,0.72); color: #fff; border: none; border-radius: 50%; font-size: 0.7rem; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; padding: 0; }
        .pi-rm:hover { background: #E85520; }

        /* ── BUTTONS ── */
        .btn-row { display: flex; gap: 0.75rem; align-items: center; }
        .btn-submit {
            padding: 0.62rem 1.6rem; background: var(--accent); color: #fff;
        ...
        .btn-cancel {
            padding: 0.62rem 1.2rem; background: transparent; color: var(--muted);
            border: 1px solid var(--border); border-radius: 8px; font-size: 0.85rem; font-weight: 500;
...
<div class="page-wrap">
    ...
    <div class="form-card">
        <form method="POST" action="{{ route('reviews.store', $race) }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="field">
                ...
                <select name="race_edition_id" class="field-input">
                    ...
                    @foreach($editions as $ed)
                        <option value="{{ $ed->id }}"
                            @selected(old('race_edition_id') == $ed->id)>
                            {{ $ed->year }}년
            ...
            <div class="field">
                ...
                <select name="distance" class="field-input {{ $errors->has('distance') ? 'field-input-err' : '' }}">
                    ...
                    @foreach($options as $d)
                        <option value="{{ $d }}" {{ old('distance') === $d ? 'selected' : '' }}>{{ $d }}</option>
                    @endforeach
                </select>
                @error('distance')
                    <p class="field-error">{{ $message }}</p>
...
(function () {
    ...
    function render() {
        ...
        wrap.classList.toggle('err', false);
    }
...
(function () {
    ...
    function available() { return MAX - files.length; }

    function updateUI() {
        counter.textContent = files.length + ' / ' + MAX;
        counter.className = 'img-counter' + (files.length >= MAX ? ' full' : '');
        addBtn.disabled = files.length >= MAX;
        zone.classList.toggle('full', files.length >= MAX);
    }

    function addFiles(list) {
        for (const f of list) {
            if (files.length >= MAX) { alert('최대 10장까지 업로드할 수 있습니다.'); break; }
