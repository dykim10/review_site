...
<body class="bg-gray-50 p-6">
    <div class="max-w-2xl mx-auto">
        <a href="{{ route('races.show', $race) }}" class="text-sm text-gray-500 hover:underline">← 대회 상세로</a>

        <div class="bg-white rounded-lg shadow p-6 mt-4">
            <h1 class="text-xl font-bold mb-1">리뷰 작성</h1>
            <p class="text-sm text-gray-500 mb-6">{{ $race->name }}</p>
            ...
            <form method="POST" action="{{ route('reviews.store', $race) }}">
                @csrf
                ...
                <div class="mb-5">
                    ...
                    <select name="distance"
                        ...
                        @foreach(['5K', '10K', '하프', '풀', '기타'] as $d)
                            ...
                            @endphp
                            @if(empty($distances) || in_array($d, $distances))
                ...
                <div class="mb-6">
                    ...
                    <textarea name="content" rows="8"
                              ...
                              placeholder="대회 참가 후기를 자유롭게 작성해주세요.">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
